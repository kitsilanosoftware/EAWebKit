...
#endif
#if ENABLE(PAGE_VISIBILITY_API)
    { "webkitHidden", DontDelete | ReadOnly, (intptr_t)static_cast<PropertySlot::GetValueFunc>(jsDocumentWebkitHidden), (intptr_t)0 THUNK_GENERATOR(0) },
#endif
...
static JSC_CONST_HASHTABLE HashTable JSDocumentTable = { 263, 255, JSDocumentTableValues, 0 };
...
public:
    ...
    {
        JSDocumentConstructor* ptr = new (JSC::allocateCell<JSDocumentConstructor>(*exec->heap())) JSDocumentConstructor(structure, globalObject);
...
#if ENABLE(TOUCH_EVENTS)
	{ "createTouch", DontDelete | Function, (intptr_t)static_cast<NativeFunction>(jsDocumentPrototypeFunctionCreateTouch), (intptr_t)11 THUNK_GENERATOR(0) },
...
const ClassInfo JSDocumentPrototype::s_info = { "DocumentPrototype", &JSC::JSNonFinalObject::s_info, &JSDocumentPrototypeTable, 0 };
...
#if ENABLE(PAGE_VISIBILITY_API)
JSValue jsDocumentWebkitHidden(ExecState* exec, JSValue slotBase, const Identifier&)
{
    JSDocument* castedThis = static_cast<JSDocument*>(asObject(slotBase));
    UNUSED_PARAM(exec);
    Document* imp = static_cast<Document*>(castedThis->impl());
    JSValue result = jsBoolean(imp->webkitHidden());
    return result;
}
...
EncodedJSValue JSC_HOST_CALL jsDocumentPrototypeFunctionCreateTouch(ExecState* exec)
{
    ...
    if (exec->hadException())
        return JSValue::encode(jsUndefined());
    int webkitRadiusX(exec->argument(7).toInt32(exec));
    if (exec->hadException())
        return JSValue::encode(jsUndefined());
    int webkitRadiusY(exec->argument(8).toInt32(exec));
    if (exec->hadException())
        return JSValue::encode(jsUndefined());
    ...
    JSC::JSValue result = toJSNewlyCreated(exec, castedThis->globalObject(), WTF::getPtr(imp->createTouch(window, target, identifier, pageX, pageY, screenX, screenY, webkitRadiusX, webkitRadiusY, webkitRotationAngle, webkitForce, ec)));
    setDOMException(exec, ec);
    return JSValue::encode(result);
}
