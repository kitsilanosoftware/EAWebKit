...
public:
    ...
    {
        JSBlobConstructor* ptr = new (JSC::allocateCell<JSBlobConstructor>(*exec->heap())) JSBlobConstructor(structure, globalObject);
...
const ClassInfo JSBlobPrototype::s_info = { "BlobPrototype", &JSC::JSNonFinalObject::s_info, 0, getJSBlobPrototypeTable };
...
EncodedJSValue JSC_HOST_CALL jsBlobPrototypeFunctionWebkitSlice(ExecState* exec)
{
    ...
    if (argsCount <= 0) {
        ...
        JSC::JSValue result = toJS(exec, castedThis->globalObject(), WTF::getPtr(imp->webkitSlice()));
