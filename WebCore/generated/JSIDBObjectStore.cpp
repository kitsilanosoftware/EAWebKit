...
public:
    ...
    {
        JSIDBObjectStoreConstructor* ptr = new (JSC::allocateCell<JSIDBObjectStoreConstructor>(*exec->heap())) JSIDBObjectStoreConstructor(structure, globalObject);
...
const ClassInfo JSIDBObjectStorePrototype::s_info = { "IDBObjectStorePrototype", &JSC::JSNonFinalObject::s_info, &JSIDBObjectStorePrototypeTable, 0 };
...
EncodedJSValue JSC_HOST_CALL jsIDBObjectStorePrototypeFunctionCreateIndex(ExecState* exec)
{
    ...
    if (exec->hadException())
        return JSValue::encode(jsUndefined());
    const String& keyPath(ustringToString(exec->argument(1).toString(exec)));
    if (exec->hadException())
        return JSValue::encode(jsUndefined());
