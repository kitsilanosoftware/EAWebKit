...
//+EAWebKitChange
//10/14/2011
// Added USE(UTFXML) and USE(LIBXML2) support.
#if USE(UTFXML)
...
//-EAWebKitChange
	private:
        ...
        bool hasError() const;
#if USE(UTFXML)
		void parseProlog(); // UTFXml parses prolog after firing startDocument
#endif
#else
public:
...
//10/14/2011
#elif USE(UTFXML)
        EA::XML::XmlReader m_stream;
        bool m_wroteText;
        String m_docContent; //Currently, UTFXml can not process chunks of XML downloaded asynchronously so we keep appending data to this string.
//-EAWebKitChange
#else
