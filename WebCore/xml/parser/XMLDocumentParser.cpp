...
//+EAWebKitChange
// Modify flow for UTFXml.
// abaldeva: Seems to me no matter the state of the parser, m_finishCalled should be set to true here.
// This is necessary at least for UTFXml based backed currently as it can not handle multiple append buffer calls.
#if USE(UTFXML)
	m_finishCalled = true;
	if(!m_parserPaused)
		end();
