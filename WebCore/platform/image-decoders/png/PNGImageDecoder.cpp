...
void PNGImageDecoder::rowAvailable(unsigned char* rowBuffer, unsigned rowIndex, int interlacePass)
{
	...
	//+EAWebKitChange
	//10/14/2011
	//Alex Mole: don't check for alpha at every single pixel, only do it once per row
	// PNG Transparency test page - http://www.fnordware.com/superpng/samples.html 
	...
	*/
	if(hasAlpha)
	{
		...
		for (int x = 0; x < width; ++x) {
			png_bytep pixel = row + (m_scaled ? m_scaledColumns[x] : x) * colorChannels;
			buffer.setRGBA(x, destY, pixel[0], pixel[1], pixel[2], pixel[3]);
	...
	else
	{
		for (int x = 0; x < width; ++x) {
			png_bytep pixel = row + (m_scaled ? m_scaledColumns[x] : x) * colorChannels;
			buffer.setRGBWithSolidAlpha(x, destY, pixel[0], pixel[1], pixel[2]);
		}
	}
	//-EAWebKitChange
}
