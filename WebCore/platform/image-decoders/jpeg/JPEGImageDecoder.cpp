    ...
    while (info->output_scanline < info->output_height) {
        ...
        for (int x = 0; x < width; ++x) {
			...
			if (info->out_color_space == JCS_RGB)
				buffer.setRGBWithSolidAlpha(x, destY, jsample[0], jsample[1], jsample[2]);
			//-EAWebKitChange
				...
				buffer.setRGBWithSolidAlpha(x, destY, jsample[0] * k / 255, jsample[1] * k / 255, jsample[2] * k / 255);
				//-EAWebKitChange
            } else {
                ASSERT_NOT_REACHED();
                return setFailed();
...
#if PLATFORM(EA)
	if (!m_reader)
		m_reader = adoptPtr(EAWEBKIT_NEW_ALIGNED(16,0,0) JPEGImageReader(this));
#else
	if (!m_reader)
