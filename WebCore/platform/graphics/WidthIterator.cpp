...
namespace WebCore {
    ...
    //1/04/2013  
#if PLATFORM(EA) 
    // As static so we can avoid changing WidthIterator.h which is shared.
    static float sKerningInsertAdjustX = 0.f; // When inserting text, this adjusts the width of the last glyph.

    static float GetEAKerningXValue(const SimpleFontData* fontData,  Glyph prevGlyph,  Glyph glyph)
    {
        float returnKerningXValue = 0.f;
        if (fontData && prevGlyph) {
            EA::WebKit::IFont* pFontEA = fontData->getEAFont();
            EA::WebKit::Kerning kerning;

            if ((pFontEA) && (pFontEA->GetKerning(prevGlyph, glyph, kerning, 0))) {    
                // Note that the rounding system used herer should stay in sync with the kerning rounding used in FontEA.cpp
                returnKerningXValue =  ceilf(kerning.mfKernX); 
            }
        }
        return returnKerningXValue;
    }
#endif
    //-EAWebKitChange
...
unsigned WidthIterator::advance(int offset, GlyphBuffer* glyphBuffer)
{
    ...
    //1/25/2012 
#if PLATFORM(EA)     
    Glyph prevGlyph = 0;            // The previous glyph is needed for kerning calls.
    float curKerningAdjustX = 0.f;   // The current glyph kerning offset to apply.
#endif  
    //-EAWebKitChange
    ...
    while (textIterator.consume(character, clusterLength)) {
        ...
        if (character == '\t' && m_run.allowTabs()) {
            ...
            //1/25/2012
#if PLATFORM(EA)     
            // Get the EA kerning offset info.
            curKerningAdjustX = GetEAKerningXValue(fontData, prevGlyph, glyph);
            prevGlyph = glyph;  // Store current glyph for next loop.
#endif 
            //-EAWebKitChange
        ...
        if (m_run.applyWordRounding() && Font::isRoundingHackCharacter(character)) {
            ...
            //1/25/2012
#if PLATFORM(EA)            
            m_runWidthSoFar += curKerningAdjustX;            
#endif            
            //-EAWebKitChange  
            ...
            {
                if (width)
                    widthSinceLastRounding += (width + curKerningAdjustX);
            }
    ...
    //+EAWebKitChange
    //1/27/2012 - If text is being inserted in the middle of a text run, we want to adjust the width with the 
    // kerning from the next glyph so that glyphs are inserted at the correct location (not just using the glyph default width).
#if PLATFORM(EA)    
    sKerningInsertAdjustX = 0.0f;    // Reset
    if ((offset < m_run.length()) && (prevGlyph != 0)) {
        SurrogatePairAwareTextIterator textIterator2(m_run.data(offset), offset,offset+1, m_run.length());
        UChar32 character2 = 0;
        if (textIterator2.consume(character2, clusterLength)) 
        {
            const GlyphData& glyphData = glyphDataForCharacter(character2, rtl, textIterator2.currentCharacter(), clusterLength);
            Glyph glyph = glyphData.glyph;
            const SimpleFontData* fontData = glyphData.fontData;

            // Add the kerning to both for this call can be used differently.
            sKerningInsertAdjustX = GetEAKerningXValue(fontData, prevGlyph, glyph);
            widthSinceLastRounding +=sKerningInsertAdjustX;
        }
    }
#endif
        //-EAWebKitChange 
...
bool WidthIterator::advanceOneCharacter(float& width, GlyphBuffer* glyphBuffer)
{
    ...
    //+EAWebKitChange
    //1/25/2012 - This advance gets the width from the glyph buffer directly so we need to add in the kerning offset.
#if PLATFORM(EA)     
    w += sKerningInsertAdjustX;
#endif
    //-EAWebKitChange
