...
void TextureMapperNode::computeTiles()
{
	...
	//+EAWebKitChange
	//8/27/2012 - Bail out early if recomputing tile size is not required otherwise it results in unnecessary allocations. 
	FloatSize size = contentSize();
	FloatRect contentRect(0, 0, size.width() * m_state.contentScale, size.height() * m_state.contentScale);

	bool shouldRecomputeTiles = (m_tileLastRecomputeRect != contentRect) || (m_tiles.isEmpty());
	if(!shouldRecomputeTiles)
		return;

	m_tileLastRecomputeRect = contentRect;
	//-EAWebKitChange
    ...
    for (size_t tileIndex = 0; tileIndex < m_tiles.size(); ++tileIndex) {
...

        {
			...
			if (m_currentContent.contentType == DirectImageContentType)
			{
				//+EAWebKitChange
				//10/17/2012
				//Need to tell context that destination rect is bigger than image size so that it can be scaled appropriately
                const IntRect& contentsRect = layer->contentsRect();
                context.drawImage(m_currentContent.image.get(), ColorSpaceDeviceRGB,contentsRect);
...
void TextureMapperNode::applyTransformAnimation(const TextureMapperAnimation& animation, const TransformOperations* from, const TransformOperations* to, double progress)
{
	...
	//+EAWebKitChange
	//8/27/2012 - Changed the copy by value to a ref otherwise it results in large number of temporary allocations.
	const TransformOperations& blended = (*to);
	//-EAWebKitChange
